...
int main()
{
    ...
    Point p1(10, 20);
    std::cout << "p1 : " << p1 << std::endl; // p1 : Point [x: 10,y: 20]
    // operator<<(std::cout,p1);

    std::cout << "------------" << std::endl;

    std::unique_ptr<Animal> animal0 = std::make_unique<Dog>("stripes", "dog1");
    std::cout << *animal0 << std::endl; // Dog [description : dog1, fur_style : stripes]

    std::unique_ptr<Animal> animal1 = std::make_unique<Bird>("white", "bird1");
    std::cout << *animal1 << std::endl; // Bird [description : bird1, wing_color : white]
    ...
    {
        std::cout << *a << std::endl; // Dog|Cat|Crow|Pigeon
    }
