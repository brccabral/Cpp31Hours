...
int main()
{
    ...
    std::cout << "sizeof(message1) : " << sizeof(message1) << std::endl; // 17, it is an array

    // strlen still works with decayed arrays
    std::cout << "strlen(message2) : " << std::strlen(message2) << std::endl; // 16

    // Prints size of pointer
    std::cout << "sizeof(message2) : " << sizeof(message2) << std::endl; // 4, it is a pointer

    // std::strcmp - signature : int strcmp( const char *lhs, const char *rhs );
    // lhs - left hand string, rhs - right hand string
    ...
    std::cout << "std::strncmp (" << string_data1 << "," << string_data2 << "," << n << ") : "
              << std::strncmp(string_data1, string_data2, n) << std::endl; // 0, the first 3 char are equal
    ...
    // it is comparing pointer result with target, not original str
    while ((result = std::strchr(result, target)) != nullptr)
    {
        ...
        ++result; // on first iteration, result will become "ry not. Do, ......."
        ++iterations;
    }
    ...
    // str2 is not const anymore (before "const char *const str")
